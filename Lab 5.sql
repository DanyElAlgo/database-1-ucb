...
CREATE TABLE SUPERIORES(
	...
	ID_INFERIOR int not null
)

INSERT INTO EMPLEADOS VALUES
(0001, 'Daniel', 'Peñaranda', 18),
(0002, 'Leticia', 'Peñaranda', 13),
(0003, 'Thiago', 'Peñaranda', 7),
(0004, 'Rolando', 'Peñaranda', 53),
(0005, 'Miriam', 'Colque', 41)
...
select *
from EMPLEADOS
where ID in (select ID_INFERIOR
			 from SUPERIORES, EMPLEADOS
			 where EMPLEADOS.ID = SUPERIORES.ID_SUPERIOR and EMPLEADOS.Nombre = 'Miriam')



select *
from EMPLEADOS
where ID in (select ID_SUPERIOR
			 from SUPERIORES, EMPLEADOS
			 where EMPLEADOS.ID = SUPERIORES.ID_INFERIOR and EMPLEADOS.Nombre = 'Thiago')